...
var RegisterEventRoutes = func(router *mux.Router) {
	...
	router.HandleFunc("/api/joinevent", Controllers.JoinEvent).Methods("GET", "OPTIONS")
	router.HandleFunc("/api/getAttendees", Controllers.GetAttendees).Methods("GET", "OPTIONS")
}
